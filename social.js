...
const loadStudentLeaderboard = async () => {
    ...
    try {
        const { data, error } = await supabase
            ...
            // REMOVED LIMIT per request: Fetching all students
...
export const loadDepartmentStudents = async (deptName) => {
    ...
    try {
        const { data, error } = await supabase
            ...
            .order('lifetime_points', { ascending: false });
            // REMOVED LIMIT per request: Fetching all students in department
...
export const renderDepartmentStudents = (deptName) => {
    ...
    // Optimization: Build large string first (Performance fix for unlimited items)
